...
public final class HlsInterstitialsAdsLoader implements AdsLoader {
  ...
  public static final class AssetList {
     ...
     */
    @Nullable
    public final SkipInfo skipInfo;
  ...
  /**
   * Resets the given ad group so final-state ads can be played again.
   *
   * <p>Played, skipped and failed ads are reset to {@link AdPlaybackState#AD_STATE_AVAILABLE} when
   * they already have a media item, or {@link AdPlaybackState#AD_STATE_UNAVAILABLE} when they don't.
   *
   * @param adGroupIndex The index of the ad group in the ad playback state.
   * @throws IllegalArgumentException if there is no ad group available for the given index.
   * @throws IllegalStateException    if called when the player {@linkplain #setPlayer(Player) is not
  ...
  public void setWithResetAdGroup(int adGroupIndex) {
    ...
    if (adPlaybackState != null) {
      ...
      // 1. add unresolved assets that werent in the list and qualify now
  ...
  /**
   * Invalidates a previously discovered {@code X-ASSET-LIST} ad group and queues it for asset-list
   * resolution again.
   *
   * <p>This is intended as a policy-neutral primitive for apps that want to re-request dynamic asset
   * lists, for example after seeking back before an interstitial opportunity. Returns {@code false}
   * if the group is not backed by a known {@code X-ASSET-LIST}.
   */
  public boolean invalidateAssetListAdGroup(int adGroupIndex) {
    Player player = checkNotNull(this.player);
    AdPlaybackState adPlaybackState = getAdPlaybackState();
    if (adPlaybackState == null) {
      return false;
    ...
    @Nullable
    AssetListData oldAssetListData =
        contentMediaSourceAdDataHolder.getAssetListDefinition(adsId, assetListTimeUs);
    if (oldAssetListData == null || oldAssetListData.interstitial.assetListUri == null) {
      return false;
    }

    AssetListData assetListData =
        new AssetListData(
            oldAssetListData.mediaItem,
            adsId,
            oldAssetListData.interstitial,
            adGroupIndex,
            /* adIndexInAdGroup= */ 0,
            oldAssetListData.targetDurationUs);
    AdGroup unresolvedAdGroup = createUnresolvedAssetListAdGroup(adGroup.timeUs, assetListData);
    adPlaybackState = adPlaybackState.withReplacedAdGroup(adGroupIndex, unresolvedAdGroup);
    ...
    if (!timeline.isEmpty() && mediaItemIndex >= 0 && mediaItemIndex < timeline.getWindowCount()) {
      ...
      long assetListWindowPositionUs =
          assetListTimeUs == Long.MAX_VALUE
              ? contentPositionUs
              : assetListTimeUs - window.positionInFirstPeriodUs;
      maybeExecuteOrSetNextAssetListResolutionMessage(
          adsId,
          timeline,
          mediaItemIndex,
          window.positionInFirstPeriodUs,
          max(0, min(contentPositionUs, assetListWindowPositionUs)));
    }
    return true;
  }

  private static AdGroup createUnresolvedAssetListAdGroup(
      long adGroupTimeUs, AssetListData assetListData) {
    Interstitial interstitial = assetListData.interstitial;
    ...
    long resumeOffsetUs =
        interstitial.resumeOffsetUs != C.TIME_UNSET
            ...
            : (interstitialDurationUs != C.TIME_UNSET ? interstitialDurationUs : 0);
    AdGroup adGroup =
        new AdGroup(adGroupTimeUs)
            .withAdCount(/* count= */ 1)
            .withAdId(interstitial.id, /* index= */ 0)
            .withAdDurationsUs(new long[] {interstitial.playoutLimitUs})
            .withContentResumeOffsetUs(resumeOffsetUs);
  ...
  private void maybeReadUnresolvedAssetListOfAdGroup(Object adsId, AdGroup adGroup) {
    ...
    if (unresolvedAssetLists != null
        && !unresolvedAssetLists.containsKey(assetListTimeUs)
        && assetListData != null) {
      unresolvedAssetLists.put(assetListTimeUs, assetListData);
      Log.d(
          "HLSTEST",
          "Re-added unresolved asset-list adsId="
              + adsId
              + " interstitialId="
              + assetListData.interstitial.id
              + " assetListTimeUs="
              + assetListTimeUs
              + " adGroup="
  ...
  public void setWithAvailableAdGroup(int adGroupIndex) {
    ...
    if (adPlaybackState != null) {
      ...
      for (int adIndexInAdGroup = 0; adIndexInAdGroup < adGroup.states.length; adIndexInAdGroup++) {
        ...
        if (adGroup.mediaItems[adIndexInAdGroup] == null) {
          Log.i("ADSMANAGER",
              " mediaItem is null for " + adIndexInAdGroup + ", cant set as available");
        }
  ...
  public boolean handleContentTimelineChanged(AdsMediaSource adsMediaSource, Timeline timeline) {
     ...
     */
    {
      if (adPlaybackState.equals(AdPlaybackState.NONE)) {
        ...
        if (isLiveMediaItem(adsMediaSource.getMediaItem(), timeline)) {
          adPlaybackState =
              adPlaybackState.withLivePostrollPlaceholderAppended(/* isServerSideInserted= */
                  false);
        }
      }
    }
    ...
    if (window.manifest instanceof HlsManifest) {
      ...
      { // log
        Log.w(
            ...
            "Content timeline update adsId="
                ...
                + window.positionInFirstPeriodUs
                + " playlistStartTimeUs="
                + mediaPlaylist.startTimeUs
                + " playlistDurationUs="
                + mediaPlaylist.durationUs
                + " targetDurationUs="
                + mediaPlaylist.targetDurationUs
                + " daterangeCount="
                + mediaPlaylist.interstitials.size()
                + " adGroupCountBefore="
                + adPlaybackState.adGroupCount
                + " unresolvedAssetListsBefore="
                + assetListCount);
      }

      /**
       * discards interstitials prior or beyond the live point; also fixes the snap inout if needed
       */
      adPlaybackState =
      ...
      { // log
        Log.d(
            "HLSTEST",
            "Mapped content timeline adsId="
                + adsId
                + " adGroupCountAfter="
                + adPlaybackState.adGroupCount
                + " removedAdGroupCount="
                + adPlaybackState.removedAdGroupCount
                + " unresolvedAssetListsAfter="
                + contentMediaSourceAdDataHolder.getUnresolvedAssetListCount(adsId));
      }
      Player player = this.player;
      ...
      if (assetListCount != contentMediaSourceAdDataHolder.getUnresolvedAssetListCount(adsId)
        ...
        if (publicPeriod.isPlaceholder) {
                  ...
                  startPositionUs, window.isLive() ? C.TIME_UNSET : window.durationUs);
          {
            Log.d(
                "HLSTEST",
                "Join/start position lookup adsId="
                    + adsId
                    + " startPositionUs="
                    + startPositionUs
                    + " windowDefaultPositionUs="
                    + window.defaultPositionUs
                    + " contentPositionUsBeforeAdjustment="
                    + contentPositionUs
                    + " selectedAdGroup="
                    + getAdGroupForLog(adPlaybackState, adGroupIndex));
          }
        ...
        }
        {
          Log.w(
              "HLSTEST",
              "Scheduling asset-list resolution from timeline update adsId="
                  + adsId
                  + " publicPeriodIsPlaceholder="
                  + publicPeriod.isPlaceholder
                  + " publicPositionInFirstPeriodUs="
                  + publicPositionInFirstPeriod
                  + " windowPositionUs="
                  + contentPositionUs);
        }
  ...
  private static String getAdIdForLog(
      @Nullable AdPlaybackState adPlaybackState, int adGroupIndex, int adIndexInAdGroup) {
    if (adPlaybackState == null
        || adGroupIndex == C.INDEX_UNSET
    ...
    return adIndexInAdGroup < adGroup.ids.length && adGroup.ids[adIndexInAdGroup] != null
        ? castNonNull(adGroup.ids[adIndexInAdGroup])
        : "?";
  }

  private static String getAdGroupForLog(
      @Nullable AdPlaybackState adPlaybackState, int adGroupIndex) {
    if (adPlaybackState == null
        || adGroupIndex == C.INDEX_UNSET
        || adGroupIndex >= adPlaybackState.adGroupCount) {
      return "?";
    ...
    AdPlaybackState.AdGroup adGroup = adPlaybackState.getAdGroup(adGroupIndex);
    String firstAdId = adGroup.ids.length > 0 && adGroup.ids[0] != null ? adGroup.ids[0] : "?";
    return "index="
        + adGroupIndex
        + ", timeUs="
        + adGroup.timeUs
  ...
  private void startLoadingAssetList(AssetListData assetListData) {
    ...
    Log.d(
        "HLSTEST",
        "Loading asset list interstitialId="
            + assetListData.interstitial.id
            + " adsId="
            + assetListData.adsId
            + " adGroup="
            ...
            + assetListData.adIndexInAdGroup
            + " uri="
            + assetListData.interstitial.assetListUri);
    getLoader()
        .startLoading(
            new ParsingLoadable<>(
  ...
  private void maybeExecuteOrSetNextAssetListResolutionMessage(
    ...
    if (loader != null && loader.isLoading()) {
      Log.d(
          "HLSTEST",
          "Asset-list resolution skipped because loader is already loading adsId=" + adsId);
      return;
    }
    cancelPendingAssetListResolutionMessage();
    ...
    long currentPeriodPositionUs = positionInFirstPeriodUs + windowPositionUs;
    {
      Log.d(
          "HLSTEST",
          "Asset-list resolution lookup adsId="
              + adsId
              + " windowIndex="
              + windowIndex
              + " positionInFirstPeriodUs="
    ...
    if (nextAssetResolution == null) {
      Log.d(
          "HLSTEST",
          "Asset-list resolution lookup found no unresolved interstitial adsId=" + adsId);
      return;
    }
                ...
                - (TARGET_DURATION_MULTIPLIER * nextAssetResolution.targetDurationUs));
    {
      Log.d(
          "HLSTEST",
          "Asset-list resolution selected adsId="
              + adsId
              + " adStartTimeUs="
              + nextAssetResolution.adStartTimeUs
              + " targetDurationUs="
              + nextAssetResolution.targetDurationUs
              + " resolutionStartTimeUs="
              + resolutionStartTimeUs
              + " currentPeriodPositionUs="
              + currentPeriodPositionUs);
    ...
    if (resolutionStartTimeUs - currentPeriodPositionUs < 200_000L) {
      ...
      Log.d("HLSTEST", "Asset-list resolution starting immediately adsId=" + adsId);
      ...
      Log.d(
          "HLSTEST",
          "Resolution message position lookup adsId="
              + adsId
              + " resolutionStartTimeUs="
              + resolutionStartTimeUs
              + " selectedAdGroup="
              + getAdGroupForLog(adPlaybackState, adGroupIndexForResolutionStartTime));
      ...
      Log.d(
          "HLSTEST",
          "Asset-list resolution scheduled adsId="
              + adsId
              + " messagePositionUs="
              + messagePositionUs
              + " messagePositionMs="
              + max(usToMs(messagePositionUs), 0));
  ...
  private RunnableAtPosition getNextAssetResolution(Object adsId, long periodPositionUs) {
    ...
    Log.i(
        "HLSTEST",
        "Searching unresolved asset lists adsId="
            + adsId
            + " periodPositionUs="
            + periodPositionUs
            + " count="
            + assetListDataMap.size());
    ...
    for (Long assetListTimeUs : assetListDataMap.keySet()) {
      ...
      Log.d(
          "HLSTEST",
          "Unresolved asset-list candidate adsId="
              + adsId
              + " interstitialId="
              + assetListData.interstitial.id
              + " assetListTimeUs="
  ...
  private long getUnresolvedAssetListWindowPositionForContentPositionUs(
    ...
    Log.d(
        "HLSTEST",
        "Unresolved asset-list position lookup contentPositionUs="
            + contentPositionUs
            + " periodPositionUs="
            + periodPositionUs
            + " periodPositionInWindowUs="
    ...
    if (adGroupIndex != C.INDEX_UNSET) {
      ...
      if (unresolvedAssets != null && unresolvedAssets.containsKey(adGroup.timeUs)) {
        ...
        long assetListWindowPositionUs = adGroup.timeUs - window.positionInFirstPeriodUs;
        Log.d(
            "HLSTEST",
            "Unresolved asset-list position lookup returning ad cue window position adsId="
                + adPlaybackState.adsId
                + " adGroup="
                + adGroupIndex
                + " adTimeUs="
                ...
                + assetListWindowPositionUs);
        return assetListWindowPositionUs;
      }
    }
    Log.d("HLSTEST", "Unresolved asset-list position lookup found no unresolved cue");
    return C.TIME_UNSET;
  }
  ...
  private boolean putAndNotifyAdPlaybackStateUpdate(Object adsId, AdPlaybackState adPlaybackState) {
    ...
    Log.d(
        "HLSTEST",
        "AdPlaybackState update adsId="
            + adsId
            + " changed="
            + !adPlaybackState.equals(oldAdPlaybackState)
            + " state="
            + adPlaybackState);
  ...
  private AdPlaybackState mapInterstitialsForLive(
    ...
    for (int i = 0; i < interstitials.size(); i++) {
      ...
      for (int j = 0; j < interstitialsAtSameStartTime.size(); j++) {
        ...
        AdPlaybackState.AdGroup insertedAdGroup = adPlaybackState.getAdGroup(insertionIndex);
        {
          Log.w(
              "HLSTEST",
              "Inserted live interstitial id="
                  + interstitial.id
                  + " adsId="
                  + adsId
                  + " adGroup="
                  + insertionIndex
                  + " count="
                  + insertedAdGroup.count
                  + " timeUs="
                  + insertedAdGroup.timeUs
                  + " contentResumeOffsetUs="
                  + insertedAdGroup.contentResumeOffsetUs);
        }
      }
    }
  ...
  /**
   * Rejects interstitiasls that are prior to the current live window, or beyond the current live
   * window. Does not reject interstitials prior to the current playback position! These get inserted
   * just fine.
   *
   * @param interstitials
   * @param adsId
   * @param mediaPlaylist
   * @param windowDefaultPositionUs
   * @param isLive
   * @return
   */
  ...
  private LongSparseArray<List<Interstitial>> filterAndSortWithResolvedStartPositions(
    ...
    for (int i = 0; i < interstitials.size(); i++) {
      ...
      boolean alreadyInserted =
          contentMediaSourceAdDataHolder.isInsertedInterstitialId(adsId, interstitial.id);
      boolean unsupportedLivePostroll = isLive && interstitial.cue.contains(CUE_TRIGGER_POST);
      if (alreadyInserted || unsupportedLivePostroll) {
        Log.d(
            "HLSTEST",
            "Ignoring interstitial id="
                + interstitial.id
                + " adsId="
                + adsId
                + " alreadyInserted="
                + alreadyInserted
                + " unsupportedLivePostroll="
                + unsupportedLivePostroll);
        continue;
      }
...
//
//        Log.i("HLSTEST", "Found adGroupIndex = " + adGroupIndex + "in map Interstitials for Vod");
//        if (adGroupIndex == C.INDEX_UNSET) {
...
//  }
  ...
  private AdPlaybackState insertOrUpdateInterstitialInAdGroup(
    ...
    if (interstitial.assetUri != null) {
      Log.d(
          "HLSTEST",
          "Interstitial uses X-ASSET-URI id="
              + interstitial.id
              + " adsId="
              + adPlaybackState.adsId
              + " adGroup="
              ...
              + adIndexInAdGroup
              + " assetUri="
              + interstitial.assetUri
              + " resumeOffsetUs="
              + resumeOffsetUs
              + " durationUs="
              + interstitialDurationUs
              + " playoutLimitUs="
              + interstitial.playoutLimitUs);
      adPlaybackState =
...
//                  .setMimeType(MimeTypes.APPLICATION_M3U8)
      ...
      /**
       * means the interstitial uses ASSET_LIST
       */
      Object adsId = checkNotNull(adPlaybackState.adsId);
      ...
      Log.d(
          "HLSTEST",
          "Interstitial uses X-ASSET-LIST id="
              + interstitial.id
              + " adsId="
              + adsId
              + " adGroup="
    ...
    public void onPositionDiscontinuity(
      ...
      if (reason == DISCONTINUITY_REASON_AUTO_TRANSITION) {
        ...
        if (newPosition.adIndexInAdGroup != C.INDEX_UNSET) {
          ...
          Log.d(
              "HLSTEST",
              "Ad started reason=AUTO_TRANSITION adsId="
                  + adsId
                  + " adGroup="
                  ...
                  + " adId="
                  + getAdIdForLog(
                  adPlaybackState, newPosition.adGroupIndex, newPosition.adIndexInAdGroup)
                  + " contentPositionMs="
        ...
        Log.d(
            "HLSTEST",
            "Triggering maybeExecuteOrSetNextAssetListResolutionMessage after seek!");
        maybeExecuteOrSetNextAssetListResolutionMessage(
            adsId,
            currentTimeline,
        ...
        if (oldPosition.adIndexInAdGroup == C.INDEX_UNSET
          ...
          Log.d(
              "HLSTEST",
              "Ad started reason="
                  + (reason == DISCONTINUITY_REASON_SEEK ? "SEEK" : "SEEK_ADJUSTMENT")
                  + " adsId="
                  + adsId
                  + " adGroup="
                  + newPosition.adGroupIndex
      ...
      if (adPlaybackState != null
        ...
        Log.d(
            ...
            "Ad completed adsId="
                ...
                + adIndexInAdGroup
                + " adId="
                + getAdIdForLog(adPlaybackState, adGroupIndex, adIndexInAdGroup));
  ...
  /* package */ static final class ContentMediaSourceAdDataHolder {
    ...
    private final Map<Object, TreeMap<Long, AssetListData>> assetListDefinitions;
    ...
    public ContentMediaSourceAdDataHolder() {
      ...
      assetListDefinitions = new HashMap<>();
    ...
    public EventListener startContentSource(Object adsId, EventListener listener) {
      ...
      assetListDefinitions.put(adsId, new TreeMap<>());
    ...
    @Nullable
    public AssetListData getAssetListDefinition(Object adsId, long assetListTimeUs) {
      TreeMap<Long, AssetListData> assetListDataTreeMap = assetListDefinitions.get(adsId);
      return assetListDataTreeMap != null ? assetListDataTreeMap.get(assetListTimeUs) : null;
    }
    ...
    public AdPlaybackState stopContentSource(Object adsId) {
      ...
      assetListDefinitions.remove(adsId);
  ...
  private class LoaderCallback
    ...
    public void onLoadCompleted(
      ...
      for (int i = 0; i < assetList.assets.size(); i++) {
        ...
        Log.d(
            "HLSTEST",
            "Asset list item interstitialId="
                + assetListData.interstitial.id
                + " adsId="
                + assetListData.adsId
                + " adGroup="
      ...
      Log.d(
          "HLSTEST",
          "Asset list resolved interstitialId="
              + assetListData.interstitial.id
              + " adsId="
              + assetListData.adsId
              + " adGroup="
